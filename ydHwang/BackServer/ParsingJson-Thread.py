...
def Make_indicat(indi, prc_lst, market, stk_nm, hourday_tp):
    ...
    # Trend
        ...
        if market == 'upbit' and hourday_tp == 'day':
            prc_lst = upbit_days_ago(prc_lst, int(indi['val']['period']), str(indi['val']['val']),market,stk_nm)
        elif market == 'upbit' and hourday_tp == 'hour':
            prc_lst = upbit_hours_ago(prc_lst, int(indi['val']['period']), str(indi['val']['val']),market,stk_nm)
...
def upbit_days_ago(df, n, val,market,stk_nm = 'krwbtc'):
    data = str(df.tail(1)['timestamp']).split('\n')[0].split(' ')
    days_ago_data = Get_DtPrc(market, str(df['timestamp'].head(1)[0]), str(data[4]), n,stk_nm)
    df['days_ago'] = days_ago_data[val].values
    return df


def upbit_hours_ago(df, n, val,market,stk_nm = 'krwbtc'):
    # 시작날짜/시간 추출
    ...
    days_ago_data = Get_HrPrc(market, srt_day_data, end_day_data, srt_time_data, end_time_data, n,stk_nm)
...
# 비트코인 일봉 기준 시세 가져오기
def Get_DtPrc(market='upbit', str_date='0', end_date='99999999', movement=0,stk_nm='krwbtc'):
    # print('market: ' + market + ' str_date: ' + str_date + ' end_date: ' + end_date)

    ws = create_connection('ws://15.164.166.28:80/BackServer_Day')
    order_packet = 'load' + '|' + market + '|' + str_date + '|' + end_date + '|' + str(movement) + '|'+str(stk_nm)
    if ws.connected:
...
# 비트코인 시간봉 기준 시세 가져오기
def Get_HrPrc(market='upbit', str_date='0', end_date='99999999', srt_time='00', end_time='23', movement=0, stk_nm ='krwbtc'):
    ws = create_connection('ws://15.164.166.28:80/BackServer_Hr')
    order_packet = 'load' + '|' + market + '|' + str_date + '|' + end_date + '|' + srt_time + '|' + end_time + '|' + str(movement) + '|' + str(stk_nm)
    print(order_packet)
    if ws.connected:
        ws.send(order_packet)
        result = ws.recv()
...
def Get_DtForeStkPrc(market='fore', stk_nm='apple', str_date='0', end_date='99999999', movement=0):
    ws = create_connection('ws://15.164.166.28:80/BackServer_Forestk_Day')
...
def Get_HrForeStkPrc(market='fore', stk_nm='apple', str_date='0', end_date='99999999', srt_time='00', end_time='23',
                     movement=0):
    ws = create_connection('ws://15.164.166.28:80/BackServer_Forestk_Hr')
...
def Parsing_Main(buy_strategy='', sell_strategy='', market='upbit', stk_nm='apple', srt_date='00000000',
    ...
    if market == 'upbit':
        ...
        if hourday_tp == 'day':  # 일봉일 경우
            Prc_history = Get_DtPrc(market, srt_date, end_date, 0,stk_nm)
        else:  # 시간봉일 경우
            Prc_history = Get_HrPrc(market, srt_date, end_date, srt_time, end_time, 0,stk_nm)
...
if __name__ == '__main__':
    ...
    result = Parsing_Main(json_data_buy, json_data_sell, 'upbit', '', '20200916', '20200918', srt_time, end_time, 'hour')
